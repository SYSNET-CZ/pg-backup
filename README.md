...
A simple docker container that runs PostGIS backups. It is derrived from kartoza/pg-backup adding the Czech locales. It is intended to be used
primarily with our [docker postgis](https://github.com/SYSNET-CZ/postgis)
docker image. By default it will create a backup once per night (at 23h00)in a 
nicely ordered directory by year / month.

* Visit our page on the docker hub at: https://registry.hub.docker.com/u/sysnetcz/pg-backup/
* Visit our page on github at: https://github.com/SYSNET-CZ/pg-backup
...
```
docker pull sysnetcz/pg-backup:latest
docker pull sysnetcz/pg-backup:9.6
docker pull sysnetcz/pg-backup:9.5
docker pull sysnetcz/pg-backup:9.4
docker pull sysnetcz/pg-backup:9.3
```
...
```
docker build -t sysnetcz/pg-backups .
```
...
```
docker run -e POSTGRES_USER=bob -e POSTGRES_PASS=secret -link db -i -d sysnetcz/pg-backup
```
...
```
docker run -e DUMPPREFIX=foo -link db -i -d sysnetcz/pg-backup
```
...
```
db:
  image: sysnetcz/postgis:9.4-2.1
  volumes:
...
Radim Jaeger (rjaeger@sysnet.com)
April 2019
